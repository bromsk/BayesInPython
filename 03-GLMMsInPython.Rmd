...
```{r knitrStart, include=F, cache = F, message=F, warning=FALSE, results = "hide"}
...
```{r, fig.cap = "Example trial locations.", echo = F}
...
```{python plotTrapLocsPy, echo = F, eval=F}
...
```{r, fig.cap = "Example trial locations.", echo = F, eval=F}
...
```{python nb1py, results='hide', message=F, warning=F}
...
```{python TRnb1py, echo = F}
## obtain TR estimate for comparison later (custom function in Setup code)
TR_1nb = getTR(results1nb)
print(TR_1nb)
```

```{python p1nb-glmFigPy, echo=F, fig.cap = "Negative binomial GLM predictions (Python)."}
...
# az.summary(idata_bmb1nb)[, 1:4]
kable(round(az.summary(idata_bmb1nb)[, 1:4], 3), caption = "Coefficient estimates from the Bayesian framework, fit using the brms package.")
